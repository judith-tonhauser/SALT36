...
WebPPL implementations of the four reasoning-based models in Section 3 of the paper. The negation model for *know* is the one in Scontras & Tonhauser (2025); the other three models are new in this paper. Each folder contains `model.wppl` (the model), `model-evaluation.R` (the script that runs the model through `rwebppl` and writes the pragmatic-listener distribution), and `data/PL.csv` (that distribution).
